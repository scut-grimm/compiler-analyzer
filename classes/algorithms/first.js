...
class First {
  ...
  getFirstSetSize(){
    ...
    for(let i=0; i<nonterminals.length; i++){
      if(this.firstSet.has(nonterminals[i]))
        size += this.firstSet.get(nonterminals[i]).size;
    }
    return size;
  }
  ...
  calFirstSet(){
    ...
    let changed = true;
    while(changed){
      let originSize=this.getFirstSetSize();
      for (let production of this.production) {
        let head = production.getHead();
        let body = production.getBody();
        if (body[0].isTerminal() || body[0].isEmpty()) {
          ...
          for (let sign of body) {
            ...
            for(let i=0; i<result1.length; i++)
              this.firstSet.add(head, result1[i])
        }
      }
