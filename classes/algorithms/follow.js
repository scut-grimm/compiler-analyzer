...
class Follow{
  ...
  putStart(s){
    if(s === this.gram.getStartSign())
      this.followSet.add(s,this.gram.getSign('$'));
  }

  putFirst(s,prod){
    let body = prod.getBody();
    for(var i=0; i<body.length; ++i){
      if(body[i]===s&&(i+1)< body.length){
            var tempS = this.gram.getSignFirstSet(body[i+1]);
            for(let j=0; j<tempS.length;++j){
              if(tempS[j].symbol!=='ε')
                this.followSet.add(s,tempS[j]);
           // }
          //}
        }
            break
  ...
  putFollow(s,prod){
    ...
    let tempS = this.gram.getSignFollowSet(head);
  ...
  caculFollow(){
    ...
    while(true){
      ...
      for(var i=0;i<nonter.length;++i){
        if(count)
          var size = this.followSet.get(nonter[i]).size;
        ...
        if(count)
          if(size === this.followSet.get(nonter[i]).size)
            ++finish;
      }
      ++count
      if (finish===nonter.length)
        break;
   }
   //console.log(count)
